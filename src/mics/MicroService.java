 ...
 * {@link #sendBroadcast(mics.Broadcast)}, {@link #sendBroadcast(mics.Broadcast)},
 ...
 * message-queue (see {@link MessageBus#register(mics.MicroService)}
...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Message, Callback> onCompleteMap;

    /**
     * @param name the micro-service name (used mainly for debugging purposes -
     *             does not have to be unique)
     */
    public MicroService(String name, CountDownLatch doneSignal) {
        this.name = name;
        this.doneSignal = doneSignal;
        this.callbackMap = new ConcurrentHashMap<Class<? extends Message>, Callback>();
        this.onCompleteMap = new ConcurrentHashMap<Message, Callback>();
    ...
    protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
    	if(MessageBusImpl.getInstance().sendRequest(r, this)){
    		onCompleteMap.put(r, onComplete);
    		return true;
    	}
        return false; 
    }
	...
	public final void run() {
		...
		while (!terminated) {
			try {
				...
				// if the message is of type RequestCompleted
				if (message.getClass() == RequestCompleted.class) {
					// use callback for RequestCompleted
					onCompleteMap.get(((RequestCompleted) message).getCompletedRequest()).call(((RequestCompleted) message).getResult());
				} else
					// use callback for message
